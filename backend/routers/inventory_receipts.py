...
def create_goods_receipt(rcv_in: schemas.GoodsReceiptCreate, db: Session = Depends(get_db)):
    ...
    update StoreInventory and InventoryItem.quantity_on_hand (mirror).
    """
    MAIN_STORE_ID = 1
    try:
        # choose store (default to MAIN if not provided)
        ...
        # Create GoodsReceipt header (if you keep that model)
        ...
        # For each item line: create GoodsReceiptItem, InventoryMovement (IN), update store_inventories and inventory_items
        for row in rcv_in.items:
            ...
            # InventoryMovement (ledger truth)
            ...
            # Update or create StoreInventory (mirror)
            ...
            if not si:
                si = models.StoreInventory(store_id=store_id, item_id=row.item_id, quantity=qty, avg_cost=row.cost_price)
                db.add(si)
            else:
                # quantity is numeric, keep sum
                si.quantity = (si.quantity or 0) + qty
                # optionally update avg_cost using weighted average, if you want
                if row.cost_price is not None:
                    prev_qty = float(si.quantity or 0) - qty
                    prev_cost = float(si.avg_cost or 0) if si.avg_cost is not None else 0.0
                    new_total = prev_qty + qty
                    si.avg_cost = (prev_cost * prev_qty + float(row.cost_price) * qty) / new_total if new_total > 0 else row.cost_price
                db.add(si)

            # Update inventory_items.quantity_on_hand (global) as sum of movements (cheap: increment)
...
def create_direct_receipt(rcv_in: schemas.GoodsReceiptCreate, db: Session = Depends(get_db)):
    """
    Directly receive stock without a Purchase Order.
    Works the same as /receipts but skips LPO validation.
    """

    # --- Ensure store exists or fallback to Main Store ---
    if not getattr(rcv_in, "store_id", None):
        main_store = db.query(models.Store).filter(models.Store.name.ilike("%main%")).first()
        if not main_store:
            main_store = models.Store(name="Main Store", location="Default")
            db.add(main_store)
            db.commit()
            db.refresh(main_store)
        store_id = main_store.id
    else:
        store_obj = db.query(models.Store).filter(models.Store.id == rcv_in.store_id).first()
        if not store_obj:
            raise HTTPException(status_code=404, detail="Store not found")
        store_id = rcv_in.store_id
    ...
    for row in rcv_in.items:
        ...
        inv_item = db.query(models.InventoryItem).filter(models.InventoryItem.id == row.item_id).first()
        if not inv_item:
            raise HTTPException(status_code=404, detail=f"Item {row.item_id} not found")

        # Create GoodsReceiptItem
        rec_item = models.GoodsReceiptItem(
            receipt_id=receipt.id,
            item_id=row.item_id,
            quantity_received=qty,
            cost_price=row.cost_price,
